...
module.exports = class Command {
	constructor() {
		this.name = 'cosmos-init';
		this.configName = 'cosmos-init-config';
	...
	* downloadBoilerplate(pkgName) {
		...
		const saveDir = path.join(os.tmpdir(), 'cosmos-init-boilerplate');
	 ...
	 * @param {Object} mapping - boilerplate config mapping, `{ simple: { "name": "simple", "package": "cosmos-boilerplate-simple", "description": "Simple cosmos app boilerplate" }}`
	...
	* askForBoilerplateType(mapping) {
	 ...
	 * @return {Object} boilerplate config mapping, `{ simple: { "name": "simple", "package": "cosmos-boilerplate-simple", "description": "Simple cosmos app boilerplate" }}`
	 */
	...
	* getTemplateDir() {
		let templateDir;
		// when use `cosmos-init --template=PATH`
	...
	* getTargetDirectory() {
	...
	getParser() {
		return yargs
			.usage('init cosmos project from boilerplate.\nUsage: $0 [dir] --type=simple')
			.option(this.getParserOptions())
			.alias('h', 'help')
			.version()
			.help();
	}
	...
	getParserOptions() {
		return {
			...
			force: {
				type: 'boolean',
				description: 'force to override directroy',
				alias: 'f',
			},
			template: {
				type: 'string',
